...
```{r, message=FALSE}
  ...
  df_init = dplyr::bind_rows(lapply(
    ...
    tidyr::separate(celltag, sep="-", into=c("A", "B", "cellcycle_measurement"), extra="drop", remove=FALSE) %>%
...
```{r,message=FALSE,warning=FALSE}
ggplot(data = df) + geom_point(aes(x=rpc, y=prediction, color=cellcycle_measurement)) + 
...
predicted_df <- data.frame(density_pred = predict(model, df), rpc=df$rpc, density_measured=df$prediction, groundtruth_cellcycle=df$cellcycle_measurement)

ggplot(data = df) + geom_point(aes(x=rpc, y=prediction, color=prediction.quality)) + 
  geom_vline(xintercept=100, color="red", linetype="dashed") + 
...
ggplot(data = predicted_df %>% dplyr::filter(rpc > 100)) +
  geom_point(aes(x=rpc, y=residuals)) +
  geom_hline(yintercept=-0.0025, color="red") +
  theme_pubclean()

table(predicted_df$residuals < -0.0025, predicted_df$groundtruth_cellcycle, predicted_df$rpc > 100)
