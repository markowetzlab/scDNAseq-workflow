...
We also highly recommend using our naming scheme for the reads, described in the other vignette. Every name consists of four parts, separated by an underscore. It basically is:
```
...
The data (subsampled to 100 cells each) are available to download at TODO link. 
...
```{r, message=FALSE}
  ...
  df = dplyr::bind_rows(lapply(
    c("results/500/PEO1_500.rds", "results/500/PEO4_500.rds"), function(x){
...
fig_cellcycle = ggplot(data = df) + geom_quasirandom(aes(x=SLX, y=cycling_activity,
  ...
  geom_point(data = df %>% dplyr::filter(cellid %in% c("000001", "000013", "000067"), SLX=="23965"), aes(x=SLX, y=cycling_activity), color="black", size=1.2, shape=21) +
